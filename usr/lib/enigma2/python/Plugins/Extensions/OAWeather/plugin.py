...
config.plugins.OAWeather.iconset = ConfigSelection(
    default="", choices=ICONSETS)
config.plugins.OAWeather.nighticons = ConfigYesNo(default=True)
config.plugins.OAWeather.cachedata = ConfigSelection(default=0, choices=[(
    0, _("Disabled"))] + [(x, _("%d Minutes") % x) for x in (30, 60, 120)])
config.plugins.OAWeather.refreshInterval = ConfigSelectionNumber(
    0, 1440, 30, default=120, wraparound=True)
config.plugins.OAWeather.apikey = ConfigText(default="", fixed_size=False)

GEODATA = ("Frankfurt am Main, DE", "8.68417,50.11552")
config.plugins.OAWeather.weathercity = ConfigText(
    default=GEODATA[0], visible_width=250, fixed_size=False)
config.plugins.OAWeather.owm_geocode = ConfigText(default=GEODATA[1])

config.plugins.OAWeather.detailLevel = ConfigSelection(
    default="default",
    choices=[
        ("default",
         _("More Details / Smaller font")),
        ("reduced",
         _("Less details / Larger font"))])
config.plugins.OAWeather.tempUnit = ConfigSelection(
    default="Celsius", choices=[
        ("Celsius", _("Celsius")), ("Fahrenheit", _("Fahrenheit"))])
config.plugins.OAWeather.windspeedMetricUnit = ConfigSelection(
    default="km/h", choices=[("km/h", _("km/h")), ("m/s", _("m/s"))])
config.plugins.OAWeather.trendarrows = ConfigYesNo(default=True)
config.plugins.OAWeather.weatherservice = ConfigSelection(
    default="MSN",
    choices=[
        ("MSN",
         _("MSN weather")),
...
class WeatherHelper:
    def __init__(self):
        ...
        try:
            ...
            config.plugins.OAWeather.owm_geocode.value = str(
                self.locationDefault[1]) + "," + str(self.locationDefault[2])
    ...
    def searchLocation(city_name, callback, session):
        ...
        try:
            ...
            # Prepare results
            results = [(item[0], float(item[1]), float(item[2]))
                       for item in geodatalist]

            # Show selection screen
            session.openWithCallback(
                lambda result: WeatherHelper._safeCallback(
                    callback, result), ChoiceBox, title=_("Select location"), list=[
                    ("{} [lon={:.3f}, lat={:.3f}]".format(
                        item[0], item[1], item[2]), item) for item in results])
    ...
    def saveFavorites(self):
        ...
        try:
            logger.info("Saving %d favorites to %s" %
                        (len(self.favoriteList), self.favoritefile))
    ...
    def showFavoriteSelection(self, session, callback):
        ...
        session.openWithCallback(
            lambda favorite: self.handleFavoriteSelection(
                favorite,
                callback) if favorite else None,
            ChoiceBox,
    ...
    def updateConfigChoices(self):
        ...
        try:
            ...
            logger.info("Updated config choices with %s items" %
                        str(len(choices)))
            ...
            try:
                if hasattr(config.plugins.OAWeather, 'weatherlocation'):
                    default_choice = [
                        (self.locationDefault, self.locationDefault[0])]
                    config.plugins.OAWeather.weatherlocation.setChoices(
                        default_choice)
    ...
    def readFavoriteList(self):
        ...
        if exists(self.favoritefile):
            try:
                ...
                logger.info("Loaded %s favorites from JSON" %
                            str(len(self.favoriteList)))
                ...
                try:
                    ...
                    logger.info("Loaded %s favorites from pickle" %
                                str(len(self.favoriteList)))
    ...
    def handleFavoriteSelection(self, favorite, callback=None):
        ...
        try:
            ...
            location = favorite[1] if isinstance(
                favorite, tuple) and len(favorite) > 1 else favorite

            # Add to favorites if not already present
            if not any(not self.isDifferentLocation(location, fav)
    ...
    def reduceCityname(self, weathercity):
        ...
        return "{}, {}".format(
            components[0],
            components[1]) if len_components == 2 else "{}".format(
            components[0])
    ...
    def convertOldLocation(self):  # deprecated: will be removed at end of 2025
        if config.plugins.OAWeather.owm_geocode.value and config.plugins.OAWeather.weathercity.value:
            if config.plugins.OAWeather.weatherlocation.value == config.plugins.OAWeather.weatherlocation.default:
                ...
                lon, lat = eval(
                    str(config.plugins.OAWeather.owm_geocode.value))
                config.plugins.OAWeather.weatherlocation.value = (
                    weathercity, lon, lat)
                config.plugins.OAWeather.weatherlocation.save()
...
config.plugins.OAWeather.weatherlocation = ConfigSelection(
    default=weatherhelper.locationDefault, choices=[])
...
class WeatherSettingsViewNew(ConfigListScreen, Screen):
    ...
    def createSetup(self):
        ...
        self.list.append(
            getConfigListEntry(
                _("Enabled :"),
                config.plugins.OAWeather.enabled))
        if config.plugins.OAWeather.enabled.value:
            self.list.append(
                getConfigListEntry(
                    _("Weather service :"),
                    config.plugins.OAWeather.weatherservice))
            self.list.append(
                getConfigListEntry(
                    _("Weather city name :"),
                    config.plugins.OAWeather.weathercity))
            self.list.append(
                getConfigListEntry(
                    _("Weather API key :"),
                    config.plugins.OAWeather.apikey))
            self.list.append(
                getConfigListEntry(
                    _("Temperature unit :"),
                    config.plugins.OAWeather.tempUnit))
            self.list.append(
                getConfigListEntry(
                    _("Wind speed metric unit:"),
                    config.plugins.OAWeather.windspeedMetricUnit))
            self.list.append(
                getConfigListEntry(
                    _("Weather icon set :"),
                    config.plugins.OAWeather.iconset))
            self.list.append(
                getConfigListEntry(
                    _("Weather icon night switch :"),
                    config.plugins.OAWeather.nighticons))
            self.list.append(
                getConfigListEntry(
                    _("Refresh interval :"),
                    config.plugins.OAWeather.refreshInterval))
            self.list.append(
                getConfigListEntry(
                    _("Cache data :"),
                    config.plugins.OAWeather.cachedata))
            self.list.append(
                getConfigListEntry(
                    _("Enable Debug :"),
                    config.plugins.OAWeather.debug))
        ...
        if current_item:
            ...
            if item_text == _("Weather city name :"):
                ...
                self.session.openWithCallback(
                    self.VirtualKeyBoardCallBack, VirtualKeyBoard, title=title)
    ...
    def keycheckCity(self, closesave=False):
        ...
        weatherhelper.searchLocation(
            weathercity, self.returnCityChoice, self.session)
    ...
    def returnCityChoice(self, selected_location):
        ...
        try:
            logger.debug(
                "Init returnCityChoice selected_location= %r" %
                (selected_location,))
            ...
            if isinstance(selected_location, tuple):
                if len(selected_location) == 3:
                    ...
                    if isinstance(coords, (tuple, list)) and len(coords) == 3:
                        ...
                        raise ValueError(
                            "Invalid coordinate format in 2-element tuple")
                else:
            ...
            config.plugins.OAWeather.owm_geocode.value = "%s,%s" % (lon, lat)
            config.plugins.OAWeather.save()
            ...
            logger.error("Error parsing city selection: %s" % str(e))
            self.session.open(
                MessageBox,
    ...
    def choiceIdxCallback(self, selected_city):
        ...
        if len(self.selected_city) >= 4:
            ...
            logger.info(
                "Die ausgewählte Stadt hat nicht genügend Informationen.")

    def saveGeoCode(self, city, longitude, latitude):
        config.plugins.OAWeather.weathercity.value = city
        config.plugins.OAWeather.owm_geocode.value = "%s,%s" % (
            longitude, latitude)
...
class WeatherHandler():
    ...
    def __init__(self):
        ...
        self.trialcounter = 0
        # 0= green (data available), 1= yellow (still working), 2= red (no data
        # available, wait on next refresh) 3=startup
    ...
    def refreshWeatherDataCallback(self, data, error):
        if error or data is None:
            ...
            if self.trialcounter < 2:
                print(
                    "[%s] lookup for city '%s' paused, try again in 10 secs..." %
                    (MODULE_NAME, self.weathercity))
                self.currentWeatherDataValid = 1
                self.refreshTimer.start(10000, True)
            elif self.trialcounter > 5:
                print(
                    "[%s] lookup for city '%s' paused 1 h, to many errors..." %
                    (MODULE_NAME, self.weathercity))
                self.currentWeatherDataValid = 2
                self.refreshTimer.start(3600000, True)
            else:
                print(
                    "[%s] lookup for city '%s' paused 5 mins, to many errors..." %
                    (MODULE_NAME, self.weathercity))
                self.currentWeatherDataValid = 2
    ...
    def reset(self, newLocation=None, callback=None):
        ...
        if self.session:
            ...
            iconpath = join(
                ICONSETROOT,
                iconpath) if iconpath else join(
                PLUGINPATH,
                "Icons")
...
class OAWeatherPlugin(Screen):
    ...
    def __init__(self, session):
        ...
        try:
            ...
            if weatherLocation in weatherhelper.favoriteList:
                self.currFavIdx = weatherhelper.favoriteList.index(
                    weatherLocation)
            else:
                # If not, use first favorite or default
                self.currFavIdx = 0 if weatherhelper.favoriteList else 0
                config.plugins.OAWeather.weatherlocation.value = weatherhelper.favoriteList[
                    self.currFavIdx] if weatherhelper.favoriteList else weatherhelper.locationDefault
                config.plugins.OAWeather.weatherlocation.save()
        except BaseException:
        ...
        self["description"] = Label(
            _('GREEN: MANAGEMENT FAVORITES | MENU: SETUP | INFO: DETAILS | HOME'))
    ...
    def getWeatherDataCallback(self):
        ...
        for day in range(1, 6):
            ...
            self["weekday%d_temp" % day].text = "%s %s|%s %s\n%s" % (
                highTemp, tempunit, lowTemp, tempunit, text)

    def keyOk(self):
        if weatherhelper.favoriteList and weatherhandler.getValid() == 0:
            self.session.open(OAWeatherDetailview,
                              weatherhelper.favoriteList[self.currFavIdx])

    def favoriteUp(self):
        if weatherhelper.favoriteList:
            self.currFavIdx = (self.currFavIdx -
                               1) % len(weatherhelper.favoriteList)
    ...
    def favoriteManagementClosed(self, result=None):
        ...
        if result:
            ...
            config.plugins.OAWeather.owm_geocode.value = "{},{}".format(
                result[1], result[2])
    ...
    def config(self):
        self.session.openWithCallback(
            self.configFinished, WeatherSettingsViewNew)
...
class OAWeatherDetailview(Screen):
    ...
    def __init__(self, session, currlocation):
        ...
        self.currFavIdx = weatherhelper.favoriteList.index(
            currlocation) if currlocation in weatherhelper.favoriteList else 0
        self.old_weatherservice = config.plugins.OAWeather.weatherservice.value
        ...
        self["version"] = StaticText(
            "OA-Weather " + str(weatherhelper.version))
    ...
    def updateSkinList(self):
        try:
            weekday = _('Today') if self.currdatehour.weekday(
            ) == datetime.today().weekday() else self.currdatehour.strftime("%a")
            self["currdatetime"].setText(
                weekday + " " + self.currdatehour.strftime("%d %b"))
    ...
    def updateMoonData(self):
        if self.moonList:
            self["moonrise"].setText(datetime.fromisoformat(
                self.moonList[self.currdaydelta][0]).strftime("%H:%M"))
            self["moonset"].setText(datetime.fromisoformat(
                self.moonList[self.currdaydelta][1]).strftime("%H:%M"))
            ...
            self["moonsetpix"].hide()
        if self.sunList:
            self["sunrise"].setText(datetime.fromisoformat(
                self.sunList[self.currdaydelta][0]).strftime("%H:%M"))
            self["sunset"].setText(datetime.fromisoformat(
                self.sunList[self.currdaydelta][1]).strftime("%H:%M"))
        else:
    ...
    def getPixmap(self, filename):
        ...
        return LoadPixmap(
            cached=True,
            path=iconfile) if exists(iconfile) else None
    ...
    def msnparser(self):
        ...
        if responses:  # collect latest available data
            ...
            sunrisestr = datetime.fromisoformat(sunrisestr).replace(
                tzinfo=None).isoformat() if sunrisestr else ""
            sunsetstr = today["almanac"].get("sunset", "")
            sunsetstr = datetime.fromisoformat(sunsetstr).replace(
                tzinfo=None).isoformat() if sunsetstr else ""
            created = current.get("created")
            currtime = datetime.fromisoformat(created).replace(
                tzinfo=None) if created else ""
            ...
            hourly = today["hourly"]
            # workaround: use value from next hour if available
            precip = str(round(hourly[0]['precip'])) + \
                " %" if len(hourly) else self.na
            windSpd = str(round(current.get('windSpd', 0))) + (
                " km/h" if config.plugins.OAWeather.windspeedMetricUnit.value == 'km/h' else " m/s")
            windDir = _(
                weatherhandler.WI.directionsign(
                    round(
                        current.get(
                            'windDir',
                            0))))
            windGusts = str(round(current.get('windGust', 0))) + (
                " km/h" if config.plugins.OAWeather.windspeedMetricUnit.value == 'km/h' else " m/s")
            uvIndex = str(round(current.get('uv', 0)))
            visibility = str(round(current.get('vis', 0))) + " km"
            shortDesc = current.get("pvdrCap", "")  # e.g. 'bewölkt'
            # e.g. "Der Himmel wird bewölkt."
            longDesc = nowcasting.get("summary", "")
            yahoocode = weatherhandler.WI.convert2icon(
                "MSN", current.get("symbol", "")).get("yahooCode")
            yahoocode = self.nightSwitch(
                yahoocode, self.getIsNight(
                    currtime, sunrisestr, sunsetstr))
            iconfile = join(iconpath, yahoocode + ".png")
            iconpix = LoadPixmap(
                cached=True,
                path=iconfile) if iconfile and exists(iconfile) else None
            hourData = []
            hourData.append([timestr,
                             press,
                             temp,
                             feels,
                             humid,
            ...
            if days:
                ...
                for index, day in enumerate(
                        days):  # collect data on future hours of current day
                    if index:
                        hourData = []
                    ...
                    moonrisestr = datetime.fromisoformat(moonrisestr).replace(
                        tzinfo=None).isoformat() if moonrisestr else ""
                    moonsetstr = almanac.get("moonset", "")
                    moonsetstr = datetime.fromisoformat(moonsetstr).replace(
                        tzinfo=None).isoformat() if moonsetstr else ""
                    for hour in day.get("hourly", []):
                        valid = hour.get("valid")
                        currtime = datetime.fromisoformat(
                            valid).replace(tzinfo=None) if valid else ""
                        timestr = currtime.strftime(
                            "%H:%M h") if currtime else ""
                        press = str(round(hour.get('baro', 0))) + " mbar"
                        tempunit = "°C" if config.plugins.OAWeather.tempUnit.value == "Celsius" else "°F"
                        temp = str(round(hour.get('temp', 0))) + " " + tempunit
                        feels = str(round(hour.get('feels', 0))) + \
                            " " + tempunit
                        humid = str(round(hour.get('rh', 0))) + " %"
                        precip = str(round(hour.get('precip', 0))) + " %"
                        windSpd = str(round(hour.get('windSpd', 0))) + (
                            " km/h" if config.plugins.OAWeather.windspeedMetricUnit.value == 'km/h' else " m/s")
                        windDir = _(
                            weatherhandler.WI.directionsign(
                                round(
                                    hour.get(
                                        'windDir',
                                        0))))
                        windGusts = str(round(hour.get('windGust', 0))) + (
                            " km/h" if config.plugins.OAWeather.windspeedMetricUnit.value == 'km/h' else " m/s")
                        uvIndex = str(round(hour.get('uv', 0)))
                        visibility = str(round(hour.get('vis', 0))) + " km"
                        shortDesc = hour.get("pvdrCap", "")  # e.g. 'bewölkt'
                        # e.g. "Der Himmel wird bewölkt."
                        longDesc = hour.get("summary", "")
                        yahoocode = weatherhandler.WI.convert2icon("MSN", hour.get("symbol", "")).get(
                            "yahooCode")  # e.g. 'n4000' -> {'yahooCode': '26', 'meteoCode': 'Y'}
                        yahoocode = self.nightSwitch(
                            yahoocode, self.getIsNight(
                                currtime, sunrisestr, sunsetstr))
                        iconfile = join(iconpath, yahoocode + ".png")
                        iconpix = LoadPixmap(
                            cached=True, path=iconfile) if iconfile and exists(iconfile) else None
                        hourData.append([timestr,
                                         press,
                                         temp,
                                         feels,
    ...
    def omwparser(self):
        ...
        if fulldata:
            ...
            for index, sunrisestr in enumerate(sunriseList):
                ...
                self.sunList.append(
                    (sunrisestr if sunrisestr else self.na,
                     sunsetstr if sunsetstr else self.na))
            ...
            if hourly:
                ...
                currday = datetime.fromisoformat(
                    timeList[0]).replace(
                ...
                for idx, isotime in enumerate(timeList):
                    ...
                    windDir = _(
                        weatherhandler.WI.directionsign(
                            round(
                                round(
                                    wDirList[idx]))))
                    windGusts = str(round(wGustList[idx])) + (
                        " km/h" if config.plugins.OAWeather.windspeedMetricUnit.value == 'km/h' else " m/s")
                    ...
                    isNight = self.getIsNight(
                        currtime, sunriseList[daycount], sunsetList[daycount])
                    yahoocode = self.nightSwitch(
                        weatherhandler.WI.convert2icon(
                            "OMW",
                            wCodeList[idx]).get("yahooCode"),
                        isNight)  # e.g. '1' -> {'yahooCode': '34', 'meteoCode': 'B'}
                    iconfile = join(iconpath, yahoocode + ".png")
                    iconpix = LoadPixmap(
                        cached=True, path=iconfile) if iconfile and exists(iconfile) else None
                    hourData.append([timestr,
    ...
    def owmparser(self):
        ...
        if fulldata:
            ...
            sunriseTs, sunsetTs = city.get("sunrise", 0), city.get(
                "sunset", 0)  # OM only supports sunrise/sunset of today
            sunrisestr = datetime.fromtimestamp(
                sunriseTs).isoformat() if sunriseTs else ""
            sunsetstr = datetime.fromtimestamp(
                sunsetTs).isoformat() if sunsetTs else ""
            # OMW does not support moonrise / moonset at all
            self.sunList, self.moonList = [], []
            hourData = []
            tempunit = "°C" if config.plugins.OAWeather.tempUnit.value == "Celsius" else "°F"
            timeTs = fulldata.get("dt", 0)  # collect latest available data
            timestr = datetime.fromtimestamp(
                timeTs).strftime("%H:%M") if timeTs else ""
            ...
            windSpd = str(round(wind.get('speed', 0))) + (
                " km/h" if config.plugins.OAWeather.windspeedMetricUnit.value == 'km/h' else " m/s")
            windDir = _(
                weatherhandler.WI.directionsign(
                    round(
                        wind.get(
                            'deg',
                            0))))
            windGusts = str(round(hourly[0].get('wind', {}).get('gust', 0))) + (
                " km/h" if config.plugins.OAWeather.windspeedMetricUnit.value == 'km/h' else " m/s")
            ...
            yahoocode = self.nightSwitch(
                weatherhandler.WI.convert2icon(
                    ...
                    weather.get(
                        "id",
                        "n/a")).get("yahooCode"),
                isNight)  # e.g. '801' -> {'yahooCode': '34', 'meteoCode': 'B'}
            iconfile = join(iconpath, yahoocode + ".png")
            iconpix = LoadPixmap(
                cached=True,
                path=iconfile) if iconfile and exists(iconfile) else None
            ...
            dayList = []
            if hourly:
                currday = datetime.fromisoformat(hourly[0].get(
                    "dt_txt", "1900-01-01 00:00:00")).replace(hour=0, minute=0, second=0, microsecond=0)
                        ...
                        weatherhandler.WI.directionsign(
                            round(
                                wind.get(
                                    'deg',
                                    0))))
                    windGusts = str(round(wind.get('gust', 0))) + (
                        " km/h" if config.plugins.OAWeather.windspeedMetricUnit.value == 'km/h' else " m/s")
                    uvIndex = ""  # OWM does not support UV-index at all
                    visibility = str(
                        round(
    ...
    def nightSwitch(self, iconcode, isNight):
        return self.YAHOOnightswitch.get(
            iconcode,
            iconcode) if config.plugins.OAWeather.nighticons.value and isNight else self.YAHOOdayswitch.get(
            iconcode,
            iconcode)
...
class OAWeatherFavorites(Screen):
    ...
    def _handleSearchResult(self, result):
        ...
        try:
            ...
            # CASE 1: Tuple with formatted string and separate data (MSN
            # format)
            ...
            # CASE 4: City name only (fallback)
                ...
                lon, lat = 0.0, 0.0
                self._showMessage(
                    _("Coordinates not available for this location"), "warning")
            ...
            logger.info("Adding new location: " + str(city_name) +
                        " (" + str(lon) + ", " + str(lat) + ")")
            self._addToFavorites(city_name, lon, lat)

        except Exception as e:
            error_msg = "Error processing result: " + \
                str(e) + " - Raw data: " + str(result)
            logger.error(error_msg)
    ...
    def _addToFavorites(self, city, lon, lat):
        ...
        try:
            ...
            for existing in self.newFavList:
                ...
                if abs(
                    existing[1] -
                    float(lon)) < 0.01 and abs(
                    existing[2] -
                        float(lat)) < 0.01:
                    self._showMessage(
                        _("Location with similar coordinates already exists"), "info")
                    return
    ...
    def onSelect(self):
        try:
            ...
            config.plugins.OAWeather.owm_geocode.value = str(
                location_data[1]) + "," + str(location_data[2])
            config.plugins.OAWeather.weatherlocation.value = (
                location_data[0], location_data[1], location_data[2])
        ...
        if confirmed:
            try:
                ...
                if current_loc == favorite:
                    ...
                    config.plugins.OAWeather.weathercity.value = weatherhelper.locationDefault[
                        0]
                    config.plugins.OAWeather.owm_geocode.value = str(
                        weatherhelper.locationDefault[1]) + "," + str(weatherhelper.locationDefault[2])
                    config.plugins.OAWeather.save()
                    configfile.save()
    ...
    def handleEditResult(self, new_name):
        ...
        if new_name and self.selected_index is not None:
            try:
                ...
                self.newFavList[self.selected_index] = (
                    new_name, old[1], old[2])
                self.pending_changes = True
                self._refreshList()
            except IndexError:
